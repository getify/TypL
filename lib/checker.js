...
var MSG = {
	...
	ERR_CALL_ARG_COUNT_UNVERIFIABLE: 133,
	ERR_CALL_ARG_SPREAD_TYPE: 134,
	...
	ERR_LOOSE_EQUALITY_UNKNOWN_TYPE: 120,
	ERR_LOOSE_EQUALITY_MIXED_TYPES: 121,
	ERR_STRICT_EQUALITY_KNOWN_MIXED_TYPES: 122,
	ERR_STRICT_EQUALITY_KNOWN_MATCHING_TYPES: 123,
	ERR_UNARY_NUMERIC_OP_TYPE: 124,
	ERR_MODULUS_OP_BOTH_TYPES: 125,
	ERR_MODULUS_OP_TYPE: 126,
	ERR_BINARY_NUMERIC_OP_TYPE: 127,
	ERR_FUNC_RETURN_TYPE: 128,
	ERR_FUNC_RETURN_SIGNATURE: 129,
	ERR_TERNARY_COND_TYPE: 130,
	ERR_SPREAD_UNKNOWN_TYPE: 131,
	ERR_SPREAD_TYPE: 132,
	ERR_LOGICAL_COND_TYPE: 135,
	ERR_IN_OP_TYPE: 136,
	ERR_INSTANCEOF_OP_TYPE: 137,
};
...
var collectTypesVisitors = {
	...
	Identifier(path) {
		...
		if (recognizedTypeIDs.includes(path.node.name)) {
			// type ID as tag in tagged-template-literal?
			//   int`42`
			//   string`hello`
			if (
				...
				markNodeType(path.parent,{ tagged: path.node.tag.name, });
			}
		...
		// any other identifier?
			...
			// NOTE: we only assume 'undef' type for an un-typed identifier
			//   if that identifier is declared in this same scope; otherwise
			//   we can't predict execution order, so we'll leave the
			//   assumption at type 'unknown' for now (may be updated on
			//   subsequent passes).
			else if (
				path.scope.bindings &&
				...
				// NOTE: no type (yet), not `undefined, non-local binding
				// TODO: mark unknown
			}
		}
	},
...
function handleCallExpression(scope,callExprNode) {
	if (T.isIdentifier(callExprNode.callee)) {
		...
		if (calleeName in nativesReturnTypes) {
			...
			// NOTE: if callee is Function/Object/Array, omitting (undetectable) signature
		}
		else {
...
function check(code) {
	...
	for (let i = 0; i < 5; i++) {
		outputMessages = outputMessages.filter(function removeErrors(msg){
