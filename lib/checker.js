...
var visitors = {
	...
	BinaryExpression: {
		exit(path,...rest) {
			// mathematical binary expression?
	...
	BinaryPlus: {
		exit(path) {
			...
			if (
				...
				handleBinaryMathematical("+",path.node);
			}
		},
	},
	BinaryMinus: {
...
function handleBinaryMathematical(opKind,exprNode) {
	...
	if (
		...
		if (typesMatch(leftType,rightType)) {
			if (
				...
				discoveredNodeTypes.set(exprNode,{ ...leftType, });
			}
			else {
