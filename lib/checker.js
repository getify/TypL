...
var collectTypesVisitors = {
	...
	AssignmentPattern: {
		exit(path) {
			...
			if (atEndOfFuncParams(path)) {
				registerFuncSignatureParams(path.parent);
			}
		},
	},
	...
	ReturnStatement: {
		exit(path) {
			...
			var funcSignature = typeSignatures.get(func);
	...
	RestElement: {
		exit(path) {
			nodeTypes.set(path.node,{ inferred: "any", rest: true, });
...
function registerFuncSignatureParams(funcNode) {
	...
	for (let param of funcNode.params) {
		...
		if (paramType) {
			if (!paramType.rest) {
				funcSignature.paramTypes.push(paramType);
			}
		}
		else {
			funcSignature.paramTypes.push({ inferred: "unknown", });
		}
	}
...
function handleBinaryNumeric(op,exprNode) {
	...
	if (
		...
		if (typesMatch(leftType,rightType)) {
			...
			nodeTypes.set(exprNode,{ inferred: "number", });
			reportTypeMismatch(`Binary \`${op}\` operation, mixed numeric operand types`,rightType,leftType);
		}
	}
	else {
		nodeTypes.set(exprNode,{ inferred: "number", });
			...
			if (sourceType && getTypeID(sourceType) != "unknown") {
				...
				if (targetType) {
					if (!typesMatch(targetType,sourceType)) {
						...
						targetSignature &&
						!signaturesMatch(sourceSignature,targetSignature)
					) {
...
function setScopeBindingSignature(scope,bindingName,signature) {
	...
	if (binding) {
		...
		// NOTE: temporary debugging output
		console.log(`Implying ${bindingName} with type signature: ${JSON.stringify(signature)}`);
	}
}
