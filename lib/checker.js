...
var MSG = {
	...
	ERR_ASSIGNMENT_DESTRUCTURING_TYPE: 215,
	ERR_ASSIGNMENT_DESTRUCTURING_SHAPE: 315,
...
var collectTypesVisitors = {
	...
	Function: {
		...
		exit(path) {
			...
			// did we *not* encounter a `return` statement in the body?
				...
				// encountered
				delete funcShape.return.default;
				funcShape.return.inferred = "undef";
			}
	...
	ArrayExpression: {
		...
		exit(path) {
			...
			var inferredShape = inferArrayShape(path.node);
	...
	ArrayPattern: {
		...
		exit(path) {
			...
			addOutputMessage({
				id: MSG.INFO_ARR_SHAPE,
				text: `Array pattern shape: '${serializeShape(arrShape)}'`,
				node: path.node,
			});
		},
	},
	...
	RestElement: {
		exit(path) {
			var isObjectRest = T.isObjectPattern(path.parent);
			var restType = isObjectRest ?
				{ inferred: "object", isRest: true, } :
				{ inferred: "array", isRest: true, };
			var restShape = isObjectRest ?
				{ type: "object", description: "{}", } :
				{ type: "array", contains: "any", description: "any[]", };

			// NOTE: rest elements can only be identifiers, not member expressions
			markNodeType(path.node,restType);
			markTypeShape(path.node,restShape);

			var binding = path.scope.getBinding(path.node.argument.name);
			if (binding) {
				let bindingType = nodeTypes.get(binding);
				let bindingTypeID = getTypeID(bindingType);
				if (bindingTypeID == "unknown") {
					markScopeBindingType(path.scope,path.node.argument.name,{ inferred: "array", },path.node.argument);
				}
				else if (bindingTypeID != restType.inferred) {
					reportUnexpectedType(
						MSG.ERR_REST_TYPE,
				...
				let bindingShape = typeShapes.get(binding);
				if (!bindingShape) {
					typeShapes.set(binding,restShape);
				}
				else if (!isShapeAssignmentAllowed(restShape,bindingShape)) {
					reportUnexpectedShape(
						MSG.ERR_REST_SHAPE,
						"Rest element shape mismatch",
						bindingShape,
						restShape,
						path.node
					);
...
function inferArrayShape(node,allowUnknowns = true) {
	var shape = {
		type: "array",
		contains: "undef",
		description: "undef[]",
	...
	if (node.elements.length > 0) {
		...
		for (let elem of node.elements) {
			if (elem) {
				...
				// inferring shape from pattern with a rest element?
				if (type && type.isRest) {
				...
				if (elemShape && elemShape.type == "array") {
					let matchingShape = foundShapes.find(function matchShape(foundShape){
						return foundShape.description == elemShape.description;
					});
					if (!matchingShape) {
						foundShapes.push(elemShape);
					}
					if (elemShape.hasUnknowns) {
						shape.hasUnknowns = true;
					}
				}
			}
...
// *****************
// TODO: rewrite all of this to account for param destructuring
// *****************
...
function handleIdentifierAssignment(scope,exprNode,sourceType,sourceShape,sourceRefNode,targetNode) {
	var sourceTypeID = getTypeID(sourceType);
	var targetBinding = scope.getBinding(targetNode.name);
	...
	if (targetBinding) {
		...
		if (sourceType && sourceTypeID != "unknown") {
			...
			if (
				...
				markScopeBindingType(scope,targetNode.name,sourceType,sourceRefNode);

				markedKnowns.add(targetBinding);

				handleShapeAssignment(scope,exprNode,sourceType,sourceShape,sourceRefNode,targetNode);
			}
			// target already has an implied type?
				...
				else if (targetType.inferred == "undef") {
				...
				else if (!currentPass.reportedNodes.has(targetNode)) {
					currentPass.reportedNodes.add(targetNode);
					reportUnexpectedType(
...
function handleDestructuringAssignment(scope,exprNode,sourceType,sourceShape,sourceRefNode,targetNode) {
	var sourceTypeID = getTypeID(sourceType);
	...
	if (T.isArrayPattern(targetNode)) {
		...
		let sourceArrayLiteral = T.isArrayExpression(sourceRefNode);
		// are destructuring pattern elements all accounted for by array literal?
		let sourceArrayLiteralSufficient = (
			sourceArrayLiteral &&
			sourceRefNode.elements.length >= targetNode.elements.length
		);

		// source array literal elements to assign?
		if (sourceArrayLiteral) {
			for (let [idx,targetElem,] of targetNode.elements.entries()) {
				// ran out of source elements to consider?
				if (idx >= sourceRefNode.elements.length) {
					break;
				}

				let targetElemExprNode = null;
				// target is identifier in a default = value assignment?
				if (T.isAssignmentPattern(targetElem)) {
					targetElemExprNode = targetElem;
					targetElem = targetElem.left;
				}
				let sourceElem = sourceRefNode.elements[idx];
				if (sourceElem) {
					handleAssignmentExpression(scope,targetElemExprNode,sourceElem,targetElem);
				}

				// let sourceElemType = nodeTypes.get(sourceElem);
				// let sourceElemShape = typeShapes.get(sourceElem);
				// handleShapeAssignment(scope,null,sourceElemType,sourceElemShape,sourceElem,targetElem);
			}

			if (
				!sourceArrayLiteralSufficient &&
				// not an default assignment of empty array literal?
				!(
					T.isAssignmentPattern(exprNode) &&
					sourceRefNode.elements.length == 0
				)
			) {
				// report unaccounted-for destructuring elements
				for (let elemIdx = sourceRefNode.elements.length; elemIdx < targetNode.elements.length; elemIdx++) {
					let targetElem = targetNode.elements[elemIdx];
					if (!currentPass.reportedNodes.has(targetElem)) {
						currentPass.reportedNodes.add(targetElem);
						reportUnexpectedType(
							MSG.ERR_ASSIGNMENT_DESTRUCTURING_TYPE,
							"Assignment destructuring type mismatch",
							"undef",
							nodeTypes.get(targetElem),
							targetElem
						);
					}
				}
		...
		if (sourceTypeID == "array") {
			if (
				...
				(
					// not part of a default assignment?
					!(
						T.isAssignmentPattern(exprNode) &&
						exprNode.left == targetNode
					) ||
					// no target shape yet?
					!currentPass.typeShapes.has(targetNode) ||
					// target shape has unknowns?
					currentPass.typeShapes.get(targetNode).hasUnknowns === true
				)
			) {
				handleShapeAssignment(scope,exprNode,sourceType,sourceShape,sourceRefNode,targetNode);
			}
	...
	// target is object destructuring pattern?
		...
		if (T.isObjectExpression(sourceRefNode)) {
			for (let [idx,targetProp,] of targetNode.properties.entries()) {
				...
				let sourcePropValue = sourceProp ? sourceProp.value : undefined;
				if (sourcePropValue) {
					handleAssignmentExpression(scope,targetPropExprNode,sourcePropValue,targetProp);
				}

				// let sourcePropValueType = nodeTypes.get(sourcePropValue);
				// let sourcePropValueShape = typeShapes.get(sourcePropValue);
				// handleShapeAssignment(scope,null,sourcePropValueType,sourcePropValueShape,sourcePropValue,targetProp);
			}
		}

		if (sourceTypeID == "object") {
			if (
				// not part of a default assignment?
				!(
					T.isAssignmentPattern(exprNode) &&
				...
				handleShapeAssignment(scope,exprNode,sourceType,sourceShape,targetNode);
			}
		}
		else if (!currentPass.reportedNodes.has(targetNode)) {
			currentPass.reportedNodes.add(targetNode);
...
function handleShapeAssignment(scope,exprNode,sourceType,sourceShape,sourceRefNode,targetNode) {
	var sourceTypeID = getTypeID(sourceType);
	...
	if (T.isIdentifier(targetNode)) {
		...
		if (targetBinding) {
			...
			if (shapeAssignable) {
				markTypeShape(targetBinding,sourceShape,/*forceOverride=*/true);
				if (exprNode) {
					markTypeShape(exprNode,sourceShape,/*forceOverride=*/true);
				}
			}
			else if (
				targetShape &&
				!currentPass.reportedNodes.has(targetNode)
			) {
				currentPass.reportedNodes.add(targetNode);
				reportUnexpectedShape(
		...
		if (sourceShape) {
			...
			for (let [idx,targetElem,] of targetNode.elements.entries()) {
				...
				if (typeof sourceShape.contains == "string") {
					...
					handleShapeAssignment(scope,targetElemExprNode,"array",sourceShape.contains,sourceRefNode,targetElem);
				}
			}
		}
		else if (!currentPass.reportedNodes.has(targetNode)) {
			currentPass.reportedNodes.add(targetNode);
			addOutputMessage({
				id: MSG.ERR_ASSIGNMENT_SHAPE,
				type: "error",
				text: "Assignment shape mismatch: could not find shape to match against destructuring pattern",
				node: sourceRefNode,
			});
		}
	}
...
function handleAssignmentExpression(scope,exprNode,sourceNode,targetNode) {
	var sourceType = nodeTypes.get(sourceNode);
	var sourceShape = typeShapes.get(sourceNode);

	// target is simple identifier?
	if (T.isIdentifier(targetNode)) {
		handleIdentifierAssignment(scope,exprNode,sourceType,sourceShape,sourceNode,targetNode);
	}
	// target is a destructuring pattern?
		...
		handleDestructuringAssignment(scope,exprNode,sourceType,sourceShape,sourceNode,targetNode);
	}
}
	...
	if (T.isIdentifier(callExprNode.callee)) {
		...
		if (calleeName in nativesReturnTypes) {
			...
			if (typeShapes.has(nativeFn)) {
				...
				markTypeShape(nativeFn,funcShape);
			}
		...
		for (let [argIdx,argType,] of callExpressionArgumentTypes.entries()) {
			...
			if (argIdx < funcShape.params.length) {
				...
				// reverse-register an unknown param type based on a known argument type?
					...
					// ***********************
					// TODO: account for destructuring
					// ***********************
...
function isShapeAssignmentAllowed(sourceShape,targetShape) {
	...
	if (sourceShape.type == "func") {
		...
		// any kind of array can be assigned to 'unknown[]' / 'any[]'
		if (["unknown","any",].includes(targetShape.contains)) {
			return true;
		}
...
function check(code, options = {}) {
	...
	multipass: for (let passCount = 1; passCount <= PASS_LIMIT; passCount++) {
		currentPass.nodeTypes = new Map();
		currentPass.typeShapes = new Map();
		currentPass.reportedNodes = new Set();
