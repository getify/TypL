...
function handleAssignmentExpression(scope,exprNode,sourceNode,targetNode) {
	...
	// target is array destructuring pattern?
		...
		for (let [idx,targetElem,] of targetNode.elements.entries()) {
			...
			let sourceElem = T.isArrayExpression(sourceNode) ?
				sourceNode.elements[idx] :
				undefined;
			handleAssignmentExpression(scope,null,sourceElem,targetElem);
		}

		if (sourceTypeID == "array") {
			// are we handling the top-level of the array pattern?
			if (exprNode) {
				let targetShape = typeShapes.get(targetNode);
				...
				if (
					...
					reportUnexpectedShape(
						MSG.ERR_ASSIGNMENT_DESTRUCTURING_SHAPE,
						"Assignment destructuring shape mismatch",
						sourceShape,
						targetShape,
						sourceNode
					);
	...
	// target is object destructuring pattern?
		...
		if (T.isObjectExpression(sourceNode)) {
			for (let [idx,targetProp,] of targetNode.properties.entries()) {
				...
				if (sourceProp) {
					handleAssignmentExpression(scope,null,sourceProp.value,targetProp);
				}
			}
		}
	}
