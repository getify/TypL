	...
	Function: {
		...
		exit(path) {
			...
			addOutputMessage({
				...
				text: `Function '${getOrInferFunctionName(path)}' shape: ${JSON.stringify(funcShape)}`,
				node: path.node,
			});
		},
	},
...
function reportUnexpectedShape(errID,label,foundShape,expectedShape,node) {
	addOutputMessage({
		id: errID,
		type: "error",
		text: `${label}: expected shape ${JSON.stringify(expectedShape)}, but found shape ${JSON.stringify(foundShape)}`,
		node,
	});
}
...
function shapesMatch(shape1,shape2) {
	...
	if (shape1.type == "func") {
		...
		for (let [idx,paramType1,] of shape1.params.entries()) {
			...
			if (
				paramShape1 &&
				!shapesMatch(paramShape1,paramShape2)
			) {
				return false;
			}
		}
