	...
	BinaryExpression: {
		exit(path,...rest) {
			...
			// equality comparison operators?
				...
				return dispatchVisitor.call(this,visitorHelpers,"BinaryEquality",[path,...rest],"exit");
			}
		},
	},
	LogicalExpression: {
...
function handleBinaryRelational(op,exprNode) {
	...
	if (typesMatch(leftTypeID,rightTypeID)) {
		if (!validIDs.includes(leftTypeID)) {
			reportUnexpectedType(`Binary \`${op}\` operation, operand types match but unexpected`,leftType,"number|string");
		}
	}
	else if (
