...
var collectTypesVisitors = {
	...
	ArrayPattern: {
		...
		exit(path) {
			var arrShape = typeShapes.get(path.node);
			var inferredShape = inferArrayShape(path.node,/*allowUnknowns=*/false);
			Object.assign(arrShape,inferredShape);
...
function handleDestructuringAssignment(scope,exprNode,sourceType,sourceShape,sourceRefNode,targetNode) {
	...
	if (T.isArrayPattern(targetNode)) {
		...
		// assign source array elements to pattern elements?
		if (T.isArrayExpression(sourceRefNode)) {
			for (let [idx,targetElem,] of targetNode.elements.entries()) {
				...
				if (sourceElem) {
					handleAssignmentExpression(scope,null,sourceElem,targetElem);
				}
	...
	// target is object destructuring pattern?
		...
		if (T.isObjectExpression(sourceRefNode)) {
			for (let [idx,targetProp,] of targetNode.properties.entries()) {
				...
				if (sourcePropValue) {
					handleAssignmentExpression(scope,null,sourcePropValue,targetProp);
				}
			...
			for (let [idx,targetElem,] of targetNode.elements.entries()) {
				...
				if (typeof sourceShape.contains == "string") {
					...
					if (T.isIdentifier(targetElem)) {
						handleIdentifierAssignment(scope,null,sourceElemType,null,sourceRefNode,targetElem);
					}
					else {
						let targetElemType =
						...
						if (!(
							...
							sourceShape.contains == "any" ||
							sourceShape.contains == targetElemType
						)) {
					...
					handleShapeAssignment(scope,null,"array",sourceShape.contains,sourceRefNode,targetElem);
				}
			}
		}
		else if (!currentPass.reportedNodes.has(targetNode)) {
