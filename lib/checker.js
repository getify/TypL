...
var collectTypesVisitors = {
	...
	Function: {
		enter(path) {
			...
			if (!typeShapes.has(path.node)) {
				...
				if (
					getTypeID(funcShape.return) == "undef" &&
					!funcShape.return.explicit
				) {
		...
		exit(path) {
			...
			if (funcShape) {
				...
				if (!(
					...
					// the return value
					else if (!isAssignmentAllowed(returnType,funcShape.return)) {
						reportUnexpectedType(
					...
					// return type shapes mismatched?
						...
						functionReturnShape &&
						returnShape &&
						!shapesMatch(returnShape,functionReturnShape)
					) {
						reportUnexpectedShape(
							MSG.ERR_FUNC_RETURN_SHAPE,
							"Return type shape mismatched",
							returnShape,
							functionReturnShape,
							path.node.argument
						);
	...
	ArrayExpression: {
		...
		exit(path) {
			...
			for (let [elemIdx,elem,] of path.node.elements.entries()) {
				let elemType = nodeTypes.get(elem);

				if (elemType) {
					arrShape.elements[elemIdx] = elemType;

					let elemShape = typeShapes.get(elem);
					if (elemShape) {
						markTypeShape(elemType,elemShape);
						shapeNodePaths.set(elemShape,path);
					}
				}
				else if (elem) {
					arrShape.elements[elemIdx] = { inferred: "unknown", };
				}
			}

			addOutputMessage({
				id: MSG.INFO_ARR_SHAPE,
				text: `Array shape: '${serializeShape(arrShape)}'`,
				node: path.node,
			});
		},
	},
...
function serializeShape(shape) {
	if (shape.type == "func") {
		...
		return `[${serializeTypesList(shape.elements)}]`;
	}
	else {
		let str = "{";
		return str + "}";
	...
	if (T.isIdentifier(targetNode)) {
		...
		if (targetBinding) {
			...
			if (sourceType && sourceTypeID != "unknown") {
				...
				// need to copy a reference to the type shape?
				if (
					!targetShape &&
					sourceShape
				) {
			...
			if (argIdx < funcShape.params.length) {
				...
				if (paramTypeID != "unknown") {
					...
					if (!isAssignmentAllowed(argType,paramType)) {
						...
						paramShape &&
						!shapesMatch(argShape,paramShape)
					) {
						reportUnexpectedShape(
					...
					if (funcParamBinding) {
						...
						if (isTaggedType(funcParamType)) {
							addOutputMessage({
								...
								text: `Implying parameter ${funcParamName} from argument, as tagged-type '${argTypeID}'${argShape ? ` (and registered shape: ${JSON.stringify(argShape)})` : ""}`,
								node: funcParam,
							});
						}
						else {
							addOutputMessage({
								...
								text: `Implying parameter ${funcParamName} from argument, as inferred-type '${argTypeID}'${argShape ? ` (and registered shape: ${JSON.stringify(argShape)})` : ""}`,
								node: funcParam,
							});
						}
					}
...
function markScopeBindingShape(scope,bindingName,shape) {
	var binding = scope.getBinding(bindingName);
	// found a scope binding with no implied type shape?
	if (binding) {
		markTypeShape(binding,shape);
	}
}
...
// Note: params are generally in (source,target) order
function shapesMatch(shape1,shape2) {
	...
	if (shape1.type == "func") {
		if (shape1.params.length != shape2.params.length) {
			return false;
		}

		for (let [idx,paramType1,] of shape1.params.entries()) {
			let paramType2 = shape2.params[idx];

			if (getTypeID(paramType1) != getTypeID(paramType2)) {
				return false;
			}

			let paramShape1 = typeShapes.get(paramType1);
			let paramShape2 = typeShapes.get(paramType2);

			if (
				paramShape1 &&
				!shapesMatch(paramShape2,paramShape1)
			) {
				return false;
			}
		}

		let returnType1 = shape1.return;
		let returnType2 = shape2.return;

		if (getTypeID(returnType1) != getTypeID(returnType2)) {
			return false;
		}
	}
	else if (shape1.type == "array") {
		if (shape1.elements.length < shape2.elements.length) {
			return false;
		}

		for (let [idx,elemType2,] of shape2.elements.entries()) {
			let elemType1 = shape1.elements[idx];
			if (getTypeID(elemType1) != getTypeID(elemType2)) {
				return false;
			}

			let elemShape1 = typeShapes.get(elemType1);
			let elemShape2 = typeShapes.get(elemType2);

			if (
				elemShape1 &&
				!shapesMatch(elemShape2,elemShape1)
			) {
				return false;
			}
		}
