...
var collectTypesVisitors = {
	...
	Identifier(path) {
		...
		// is identifier anything other than a function's lexical name identifier?
		else if (!(
			T.isFunction(path.parent) &&
			path.parent.id &&
			path.node != path.parent.id
		)) {
						...
						if (
							...
							for (let [argIdx,arg] of callExpressionArguments.entries()) {
								if (argIdx < funcSignature.params.length) {
									...
									// register an unknown param type based on a known argument?
...
function signaturesMatch(signature1,signature2) {
	...
	if (signature1.type == "func") {
		...
		for (let [idx,paramType1] of signature1.params) {
