		...
		if (calleeName in nativesReturnTypes) {
			...
			if (funcSignature) {
				...
				if (
					...
					console.error(`Expected ${funcSignature.params.length} arguments, found ${callExpressionArgumentTypes.length}`);
				}
				else {
							...
							// reverse-register an unknown param type based on a known argument?
								...
								if (funcParamBinding) {
									...
									if (isTaggedType(funcParamType)) {
										console.log(`Implying parameter ${funcParamBinding.identifier.name} from argument, as tagged-type '${argTypeID}'${argSignature ? ` (and registered signature: ${JSON.stringify(argSignature)})` : ""}`);
									}
									else {
										console.log(`Implying parameter ${funcParamBinding.identifier.name} from argument, as inferred-type '${argTypeID}'${argSignature ? ` (and registered signature: ${JSON.stringify(argSignature)})` : ""}`);
									}
								}
							}
						}
...
function setScopeBindingType(scope,bindingName,type) {
	...
	if (
		...
		if (isTaggedType(type)) {
			console.log(`Implying ${bindingName} as tagged-type '${typeID}'`);
		}
		else {
			console.log(`Implying ${bindingName} as inferred-type '${typeID}'`);
		}
	}
}
