	...
	BinaryPlus: {
		exit(path) {
			...
			var exprType =
				...
				(extractTypeID(leftType) != "unknown") ? leftType :
				(extractTypeID(rightType) != "unknown") ? rightType :
				{ inferred: "unknown", };

			discoveredNodeTypes.set(path.node,exprType);
			console.log(`Binary Expression: '${extractTypeID(exprType)}' (type '${extractTypeID(leftType)}' + type '${extractTypeID(rightType)}')`);
		},
	},
};
...
function reportTypeMismatch(label,expectedType,foundType) {
	var expected = extractTypeID(expectedType);
	var found = extractTypeID(foundType);
