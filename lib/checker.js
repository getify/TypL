...
var visitors = {
	...
	ConditionalExpression: {
		exit(path) {
			...
			if (
				!condType ||
				extractTypeID(condType) != "bool"
			) {
				reportUnexpectedType("Ternary expression, unexpected condition type","bool",condType);
			}

			var thenType = discoveredNodeTypes.get(path.node.consequent);
			var elseType = discoveredNodeTypes.get(path.node.alternate);

			if (typesMatch(thenType,elseType)) {
				if (
					isTaggedType(thenType) ||
					isTaggedType(elseType)
				) {
					discoveredNodeTypes.set(path.node,{ tagged: extractTypeID(thenType), });
				}
				else {
					discoveredNodeTypes.set(path.node,{ inferred: extractTypeID(thenType), });
				}
			}
		},
	},
